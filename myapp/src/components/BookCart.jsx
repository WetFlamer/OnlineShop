...
const BookCart = ({bookId, name, author, poster, left, price, description}) => {
  ...
  const handleAddtoCart = async () => {
   if(token) {
    ...
    alert('Для того что бы добалять книги в корзину, вам нужно зарегистрироваться!')
   }
  }
  useEffect(() => {
  ...
  return (
    <div className={styles.books}>
      <div className={styles.booksBox}>
        ...
        <button disabled={disabled} onClick={handleAddtoCart} className={styles.addCart}>{disabled === true ? 'В корзине' : 'В корзину'} </button>
